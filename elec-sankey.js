...
export function mixHexes(hex1, hex2, ratio = 0.5) {
    if (ratio > 1 || ratio < 0) {
 ...
 */
let ElecSankey = class ElecSankey extends LitElement {
    constructor() {
        super(...arguments);
        this.unit = "kWh";
    ...
    _recalculate() {
         ...
         */
        const genTotal = generationTrackedTotal +
            (this._phantomGenerationInRoute
                ? this._phantomGenerationInRoute.rate
                : 0);
    ...
    _generationInFlowWidth() {
        return this._rateToWidth(this._generationTrackedTotal() + this._generationPhantom());
    }
    ...
    _generateLabelDiv(_id, icon, _name, valueA, valueB = undefined) {
    ...
    renderGenerationToConsumersFlow(x0, x1, y1, x2, y2, svgScaleX = 1) {
        ...
        const fanOutWidth = totalGenWidth + (count - 1) * GENERATION_FAN_OUT_HORIZONTAL_GAP;
        ...
        const generatedFlowPath2 = genToConsWidth > 0
            ? renderFlowByCorners(x0 + totalGenWidth, TERMINATOR_BLOCK_LENGTH - PAD_ANTIALIAS, x0 + totalGenWidth - genToConsWidth, TERMINATOR_BLOCK_LENGTH - PAD_ANTIALIAS, x1, y1, x2, y2, "generation")
            : svg ``;
        const svgRet = svg `
    ${svgArray}
    ${generatedFlowPath2}
    ...
    renderGenerationToGridFlow(x0, y0, x10, y10) {
        const width = this._generationToGridFlowWidth();
        if (width === 0) {
            return svg ``;
        }
        const generatedFlowPath = renderFlowByCorners(x0 + width, y0, x0, y0, x10, y10 + width, x10, y10, "generation");
        return svg `
    ${generatedFlowPath}
    <rect
      class="generation"
  ...
  `;
    ...
    renderGridInFlow(topRightX, topRightY, svgScaleX = 1) {
        if (!this.gridInRoute) {
    ...
    >
      ${this._generateLabelDiv(this.gridInRoute.id, mdiTransmissionTower, undefined, rateA, rateB)}
    </div>`;
        const svgRet = svg `
    <rect
      class="grid"
  ...
  `;
    ...
    renderGenInBlendFlow(y1, endColor) {
  ...
  `
    ...
    renderGridInBlendFlow(y2, endColor) {
  ...
  `;
    ...
    _renderBlendedFlowPreFanOut(y4, y5, color) {
        const svgRet = svg `
    <rect
  ...
  `;
    ...
    _renderConsumerFlows(y6, y7, color, svgScaleX) {
        const divRetArray = [];
        const svgRetArray = [];
        const xLeft = 0;
        ...
        for (const key in this.consumerRoutes) {
            if (Object.prototype.hasOwnProperty.call(this.consumerRoutes, key)) {
                [divRow, svgRow, yLeft, yRight] = this._renderConsumerFlow(xLeft, yLeft, xRight, yRight, this.consumerRoutes[key], color, svgScaleX, i++);
    ...
    _rateInBlendColor() {
        return mixHexes(this._gridColor(), this._genColor(), this._gridBlendRatio());
    }
    _calc_xy() {
        ...
        const y1 = Math.max(TERMINATOR_BLOCK_LENGTH + radiusGenToConsumers - widthGenToConsumers / 2, TERMINATOR_BLOCK_LENGTH + radiusGenToGrid - widthGenToGrid / 2);
        const x1 = x0 + widthGenToGrid + widthGenToConsumers / 2 + radiusGenToConsumers;
        const x2 = x1;
    ...
    render() {
        ...
        const generationToGridFlowSvg = this.renderGenerationToGridFlow(x0, y0, x10, y10);
        ...
        const blendedFlowPreFanOut = this._renderBlendedFlowPreFanOut(y1, y5, blendColor);
        ...
        const [genInFlowDiv, genInFlowSvg] = this.renderGenerationToConsumersFlow(x0, x1, y1, x2, y2, svgScaleX);
        const [consOutFlowsDiv, consOutFlowsSvg, y8] = this._renderConsumerFlows(y1, y5, blendColor, svgScaleX);
